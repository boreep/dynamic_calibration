...
% -----------------------------------------------------------------------
% [新增] 禁飞区 (No-Fly Zone) 设置
% -----------------------------------------------------------------------
nfz.enabled = true;        % [Flag] 是否开启禁飞区检测 (true/false)
nfz.check_bodies = {'Link6', 'Link3'}; % 需要检测的部位 (末端和手肘)
% 定义禁飞区的几何边界 (交集逻辑：同时满足以下所有条件则违规)
% 例子：z > 0 且 -10 < x < 10
nfz.z_min = 0.05;          % 禁入条件1: Z > 0.05 (留一点安全余量)
nfz.x_range = [-0.13, 0.13];   % 禁入条件2: -10 < X < 10
...
% ----------------------------------------------------------------------
% %{
pathToFolder = 'trajectory_optmzn/rm_opt_traj/';
